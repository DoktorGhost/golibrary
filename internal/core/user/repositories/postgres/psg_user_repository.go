...
func (s *UsersRepository) CreateUser(user dao.UserTable) (int, error) {
	var id int
	query := `INSERT INTO users.users (username, password_hash, full_name) VALUES ($1, $2, $3) RETURNING id`
...
func (s *UsersRepository) GetUserByID(id int) (dao.UserTable, error) {
	...
	query := `SELECT username, password_hash, full_name FROM users.users WHERE id = $1`
...
func (s *UsersRepository) UpdateUser(user dao.UserTable) error {
	query := `UPDATE users.users SET username = $1, password_hash = $2, full_name = $3 WHERE id = $4`
...
func (s *UsersRepository) GetUserByUsername(username string) (dao.UserTable, error) {
	var result dao.UserTable
	query := `SELECT id, username, password_hash, full_name FROM users.users WHERE username = $1`
